...
//        }
    ...
    void initViews(){
        ...
        mCameraView.setCameraIndex(CameraBridgeViewBase.CAMERA_ID_FRONT);//前置摄像头 CAMERA_ID_FRONT 后置摄像头 CAMERA_ID_BACK
        initCameraView(false);
    }

    void initCameraView(final boolean isBack){
            ...
            public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame, Rect face, Point[] keyPoints) {
                ...
                if (f.length>0){
                    for (int i = 0; i < f.length; i++) {
                        face.x = f[i].x;
                        face.y = f[i].y;
                        face.width = f[i].width;
                        face.height = f[i].height;
...
//                                Log.e("keyPoints:", keyPoints[i]+"");
//                            }

                            for (Point p : keyPoints){
                                Imgproc.circle(mRgba, new Point((float) p.x + face.x, (float) p.y + face.y), 1, FACE_RECT_COLOR, 1);
                            }
